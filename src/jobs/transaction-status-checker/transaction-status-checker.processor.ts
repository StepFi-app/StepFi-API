...
export class TransactionStatusCheckerProcessor extends WorkerHost {
  ...
  private async applyFollowUpActions(
    ...
    if (transaction.type === TransactionType.VENDOR_APPROVE || metadata?.action === 'approve') {
      return this.updateVendorStatus(metadata?.vendorId, 'approved');
    }

    if (transaction.type === TransactionType.VENDOR_SUSPEND || metadata?.action === 'suspend') {
      return this.updateVendorStatus(metadata?.vendorId, 'suspended');
    }
  ...
  private parseTransactionMetadata(xdr?: string | null): TransactionMetadata | null {
    ...
    try {
      ...
      if (functionName === 'approve_vendor') {
        const vendorId = (nativeArgs[1] ?? nativeArgs[0]) as string;
        return { vendorId, action: 'approve' };
      }

      if (functionName === 'suspend_vendor') {
        const vendorId = (nativeArgs[1] ?? nativeArgs[0]) as string;
        return { vendorId, action: 'suspend' };
      }
  ...
  private async updateVendorStatus(
    ...
    if (updateByIdError || !updatedById || updatedById.length === 0) {
      await db
        .from('vendors')
        .update({ status: newStatus, updated_at: new Date().toISOString() })
        .eq('wallet_address', vendorId);
    }
    ...
    this.logger.log(
      ...
      `Vendor ${vendorId} status updated to ${newStatus}`,
    );

    return { vendorId, vendorStatus: newStatus };
  }
